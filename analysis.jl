...
df = vcat([@pipe CSV.read("results/data_m$m.csv", DataFrame) |> 
        ...
        filter(:act_corrs => x -> !isnan(x), _) for m ∈ ms]...)
df.sparsity = compute_sparsity(df)

res = lm(@formula(act_corrs ~ m + a + c + g1 + g1_std), df)
CSV.write("results/regtable.txt", DataFrame(coeftable(res)))
...
savefig(mainfig, "figures/mainfig.png")
...
crossfig_p = plot(plts..., layout=(6,4), size=(900,1200), legend=false, dpi=500)
